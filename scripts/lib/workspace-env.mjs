...
const loadedWorkspaceRoots = new Set();

export function resolveWorkspaceRoot(fromModuleUrl, levelsUp) {
  const currentDir = path.dirname(fileURLToPath(fromModuleUrl));
  return path.resolve(currentDir, ...Array(levelsUp).fill(".."));
}

export function loadWorkspaceEnvFromModule(fromModuleUrl, levelsUp) {
