...
- uploader 不持有任何 S3/R2 凭据，所有路径审批与 presigned URL 都由 `internal-site` 签发
...
- 默认断点续传
- 已完整提交的 frame 会直接跳过
- 未完成的 frame 会先清空旧 pending revision，再重新申请一组 10 分钟 presigned PUT URL
- 仅网络类问题会重试，配置错误不会盲重试

真正上传流程现在是：

1. `group-upload-start`
2. 对每个 frame 执行 `prepare -> presigned PUT 上传 -> commit`
3. 全部 frame 成功后 `group-upload-complete`
