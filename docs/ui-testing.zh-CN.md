...
Viewer 模式切换使用 220ms 画布交叠淡入淡出，退出画布在过渡期间保留已解码图像，完成后卸载；A/B 内部切换变量仍立即显示。详情栏与热图分析栏用 240ms 布局过渡，详情开关保持原图节点。系统减少动态效果偏好会取消这些过渡。`viewer.spec.ts` 检查实际中间帧、退出层不可交互、连续反向切换、过渡完成后的卸载，以及详情栏展开和收起时的连续尺寸变化。
