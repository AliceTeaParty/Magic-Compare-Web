...
- 仅当 demo 可见且对象存储配置齐全时，继续 `db:seed`
- 启动 `internal-site`

说明：

- 基础 `docker-compose.yml` 默认拉取 GHCR `v2.0.1`；升级时用 `MAGIC_COMPARE_INTERNAL_SITE_IMAGE` 显式指定版本标签或 digest。发布流程只推送版本标签，没有 `latest` 标签。
