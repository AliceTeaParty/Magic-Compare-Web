...
执行命令、设备覆盖、截图基准更新和验证边界见 [UI 回归测试](ui-testing.zh-CN.md)。

- [x] 删除舞台外层多余 Stack，避免 spacing 清除自动居中 margin。
- [x] 舞台测量保留小数，避免整数取整留下细缝；使用中性明暗底色，把手接入主题；图像检查区域使用直角裁切，避免圆角遮挡与旋转边缘接缝。
- [x] 主题控件关闭 Firefox 原生表单状态恢复，避免刷新时覆盖 SSR 的 disabled 属性并触发 hydration 错误。
- [x] 手机胶卷使用原生触摸滚动，保留鼠标拖动与辅助滚动条。
- [x] 增加跨浏览器交互、工作台保存/失败、上传恢复、静态导出和视觉 CI 检查。
- [x] PR 使用跨浏览器矩阵，手机覆盖管理与 Viewer；上传仅覆盖桌面。补充空数据入口与全页视觉基准，维护 UI 对应用例表。
- [x] 项目筛选/排序提示不拦截菜单选项；胶卷键盘和把手操作中止上一轮惯性。
- [x] WebKit 缩略图生成回退 PNG 时使用真实格式扩展名，避免上传提交校验失败。
- [ ] 发布前在真实 iPhone 验收胶卷连续/斜向拖动和动态地址栏变化；模拟器结果不能代替这项。
...
- 扫描完成后，配对表是上传页的主工作区，Group 信息是 sticky supporting pane。上传工作台仅支持桌面，手机显示不支持提示；配对状态是只读结果，不使用看似可点击的 Chip。
- 新图组统一以 A/B 打开，复用 `DEFAULT_VIEWER_MODE`；上传页删除自定义默认模式，旧 payload 字段在服务端归一化。
...
- 导航图标反馈必须对应动作语义，并只变换图标本身：上传向上、新建旋入、工作区聚合、Case 轻微展开。不要给全部目的地套同一种缩放或弹跳，也不能改变 indicator 槽位与标签坐标。
- 部署入口始终占用固定导航位置并使用主题主色。它执行不带 `caseId` 的全站部署；运行期间保持可点击，用于重新展开当前任务，不会重复启动构建。空 Group 页面和非 Case 路由不会改变可用性。
...
- 长时间部署不能只显示一个等待 toast。全局外壳持有可恢复的部署任务，立即展示阶段、已用时间和真实上传计数；未知总量的构建阶段使用不确定进度，不估算百分比。任务面板通过 Portal 固定到视口，收起后导航入口仍显示运行状态，重复点击只重新展开同一任务。
- 浏览器通知权限不能绑在部署主动作上。主动作只负责开始任务并显示站内进度；系统通知仅在权限已经授予且页面位于后台时发送，避免权限弹窗和结果提示的出现时机混在一起。
...
- Viewer 模式专属控件按三联按钮的空间顺序进出固定槽位：前进时旧控件向左退出、后退时反向；退出快于进入，并服从 `prefers-reduced-motion`。模式切换不能重排 header、主图或胶片条。
