...
- 未选择目录时只显示素材目录和选择文件夹按钮；扫描完成后，左侧展示配对工作区，右侧以与项目设置一致的 336px 卡片展示素材目录、目标 Case、后缀映射、Group 信息及单列流程 `素材 -> 配对 -> 生成 -> 上传`。
- 上传只在桌面浏览器提供目录选择和配对工作台；窄屏页面直接提示使用桌面浏览器。
- 选择本地目录，推荐 Chrome / Edge 使用 `showDirectoryPicker()`。
- 浏览器不支持目录选择 API 时，退回 `<input webkitdirectory>`。
- 扫描平铺或常见 before / after 目录结构。目标项目可以键入筛选。
- 从文件或目录后缀推断 `Before`、`Src`、`After`、`Rip`、`Flt`、`NoDeband`、`Degrain` 等对比列；文件名里的显式后缀优先于目录提示。
- `out` / `output` / `after` 作为主 `After`；没有这些变量时优先用 `rip`，并把 `flt` / `filter` / `filtered` 保留为独立比较列。遇到非标准命名时，可填写 Before、After 首选后缀后在同一目录重新配对，例如 `rip` 和 `output`。
...
- 左侧配对区可展开单行预览，并在上传前拖拽调整 frame 顺序。
- 新图组固定以 A/B 打开；上传页不再提供默认模式设置。
...
- 配对区缩略图只在行接近视口或展开时创建 object URL，并在卸载时释放。
- 重排序、Frame 标题模式、列名修改、Heatmap 参考变化都会使预检结果失效，保证远端 job 的输入哈希与当前计划一致。
- 上传 commit 仍串行执行，减少 SQLite 写入冲突。
...
- 推断标题会按图组的 20 字限制截短，优先在英文单词边界结束；原目录名仍完整显示，标题可在上传前编辑。
