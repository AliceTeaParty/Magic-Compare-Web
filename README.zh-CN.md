...
<p align="center">
  ...
  <a href="./docs/web-uploader.zh-CN.md">Web 上传文档</a>
</p>
...
- Web 上传到公开发布的链路已经打通，可直接从本地素材生成 review case。
...
- `docker-compose.yml` 示例会拉起：
  - `internal-site`
  - 在应用启动前执行 `db:push` 和（配置了外部存储时）`db:seed` 的一次性 `internal-site-init`
- 服务器 Docker 部署现在只需要：
  - `docker-compose.yml`
  - `.env`
  - 已发布的运行时镜像
...
- 内部原图、缩略图和 heatmap 统一由 `MAGIC_COMPARE_S3_*` 配置的外部对象存储承载（Cloudflare R2 或其他 S3-compatible 服务）
...
1. 从内部站打开 `/upload`，或在 Case Workspace 中通过 `/upload?case=<caseSlug>` 预选目标 Case。
2. 选择本地目录，在右侧配对预览中检查列、顺序和问题。
3. 浏览器 worker 生成缩略图和缺失 heatmap。
4. 走 `group-upload-start -> frame prepare -> presigned PUT -> frame commit -> group-upload-complete`。
5. 由内部站更新 case/group metadata，并把已提交 frame 切换到新的对象存储 revision。
...
- 展示 case 工作区
- 展示内部 group viewer
- 提供 Web 上传工作台和 frame-level upload API
- 调整 case 下的 group 顺序
...
<details>
<summary><strong>📥 上传流程细节</strong></summary>

当前推荐流程是“浏览器扫描 + worker 生成 + 对象存储直传”。

1. `/upload` 在浏览器中扫描本地目录并生成配对计划。
2. 右侧配对预览可修改备选列名、拖拽调整 frame 顺序，并选择全局 heatmap 参考。
3. worker 生成缩略图和缺失 heatmap，避免阻塞 UI。
4. `group-upload-start` 创建或恢复 group 上传任务。
5. 每个 frame 执行 `prepare -> presigned PUT 上传 -> commit`。
6. 全部 frame 提交后调用 `group-upload-complete` 完成整组上传。
7. 如需放弃，`group-upload-cancel` 会取消 active job 并删除未提交 pending 前缀。
...
- Web 上传工作台是当前主上传路径；目录选择优先推荐 Chrome / Edge。
- 站内仍没有讨论、打分、标注或评审流程。
